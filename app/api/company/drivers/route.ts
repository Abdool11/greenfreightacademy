...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < drivers.length; i++) {
    const d = drivers[i];
    const idx = i + 1; // 1-indexed for user-friendly error messages
    ...
    // Duplicate check (within company + within this batch)
    ...
    existingMobiles.add(normalisedMobile); // prevent duplicates within the same batch
    created.push({
      id: data.id,
