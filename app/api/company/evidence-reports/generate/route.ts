...
export async function POST(req: NextRequest) {
  ...
  const { data: report, error } = await supabaseAdmin.from("evidence_reports").insert({ company_id: session.companyId, control_number: controlNumber, report_type: reportType, reporting_period_start: periodStart, reporting_period_end: periodEnd, filters_json: body.filters || {}, snapshot_json: snapshot, sha256_checksum: sha256, generated_by: session.userId }).select("id,control_number,sha256_checksum,generated_at").single();
  if (error) return NextResponse.json({ error: "Could not create controlled evidence snapshot" }, { status: 500 });
  await supabaseAdmin.from("evidence_report_events").insert({ report_id: report.id, event_type: "generated", actor_id: session.userId });
