  ...
  const handleDeploy = async (quoteId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const driversData = await driversRes.json();
        const allEnrolments: string[] = (driversData.drivers ?? []).flatMap((d: { enrolments: Array<{ id: string; campaign_id?: string }> }) =>
          d.enrolments.filter((e) => e.campaign_id === quoteId).map((e) => e.id)
        );
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                  {quotes.map(quote => (
                    <div key={quote.id} style={{ background: "#0d1520", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "0.875rem", padding: "1.25rem 1.5rem" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
                        ...
                        <div style={{ display: "flex", gap: "0.625rem" }}>
                          ...
                          {(quote.status === "deployed" ||
                            (quote.items_json && quote.items_json.length > 0 && quote.items_json.every((item) => item.deployedAt))) && (
                      ...
                      {(quote.status === "paid" || quote.status === "approved" || quote.status === "deployed") &&
                        quote.items_json && quote.items_json.length > 0 &&
                        quote.items_json.some((item) => !item.deployedAt) && (
                        <QuoteDriverForm
                          quoteId={quote.id}
                          itemsJson={quote.items_json}
