    ...
    <div className="min-h-screen bg-[#0a1628] text-white">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-xl border border-slate-700/50 bg-[#111f3a] p-6"><div className="flex items-center gap-2 mb-1"><FileText size={18} className="text-purple-300" /><h2 className="font-semibold">Quote terms</h2></div><p className="text-slate-400 text-sm mb-5">These fields are copied into each quote snapshot when it is issued.</p><div className="grid md:grid-cols-3 gap-4"><div>{textField("quote_validity_days", "Quote validity (days)")}</div></div><div className="mt-4"><label style={labelStyle}>Payment terms</label><textarea style={{ ...inputStyle, minHeight: "70px", resize: "vertical" }} value={config.quote_payment_terms || ""} onChange={(e) => update("quote_payment_terms", e.target.value)} /></div><div className="mt-4"><label style={labelStyle}>Optional commercial / procurement note</label><textarea style={{ ...inputStyle, minHeight: "70px", resize: "vertical" }} value={config.quote_terms_note || ""} onChange={(e) => update("quote_terms_note", e.target.value)} placeholder="Optional — e.g. payment reference or procurement guidance" /></div></section>
        </div>
        <div className="mt-6 flex justify-end"><button onClick={save} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[#2ecc71] px-5 py-3 font-bold text-slate-950 disabled:opacity-60">{saving ? <Loader2 className="animate-spin" size={17} /> : saved ? <CheckCircle2 size={17} /> : <Save size={17} />}{saving ? "Saving…" : saved ? "Saved" : "Save formal quote settings"}</button></div>
      </main>
    </div>
  );
}
