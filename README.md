...
Release 16 adds `supabase/migrations/20260916_r16_auto_certificate_issue.sql` after Releases 11–15. It replaces the new qualifying-completion `PENDING_REVIEW` path with an atomic GFA-only issuance procedure. A signed BetterDriver completion assertion remains the upstream evidence; GFA allocates one canonical certificate number, records the automatic issuance and renders/stores the private official PDF before the exact-number public registry reports the certificate as active. The Version 2 contract remains disabled until its synthetic Preview acceptance has passed. This release is published on `feature/gfa-auto-certification` at `0bcc1c5`; it is not a claim of production deployment.
...
Apply the Release 17 migration in a non-production environment first. Verify that exactly one recognised PTDP course record exists before migration; the migration stops with a clear error if it cannot find `ptdp` or `professional-truck-driver`. After deployment, configure only synthetic Preview credentials and fixtures, keep WhatsApp/email mocked or disabled, then run:
...
Release 16 adds `supabase/migrations/20260916_r16_auto_certificate_issue.sql` and changes the GFA Version 2 completion-evidence path so qualifying new Professional Truck Driver Program completions no longer create `PENDING_REVIEW` records. The trusted BetterDriver learning flow continues to apply the approved course rule before it emits `bd.learning_completion_evidence.v1`: every required module is complete and every required quiz score is at least **4/5**. GFA continues to verify the signed event and opaque mapping, then atomically records the event, allocates the canonical `GFA-YYYY-########` number and creates one `ISSUED` certification. The application renders and stores the official private PDF immediately afterwards, then marks the record `active` for exact-number public verification.

The database function is idempotent by the signed source-event identifier. A retry returns the original certificate and never allocates a second number. If private PDF storage has a transient failure, the certificate remains `ISSUED` with `pending_document` status; retrying the same authenticated completion event reattempts PDF rendering. The public registry returns `not_found` until the private PDF path exists and the certificate is active, preventing premature public verification.

Apply Release 16 after Releases 11–15. In a non-production Preview first, configure a synthetic signed `GFA_TEST_CERTIFICATE_COMPLETION_ASSERTION` and the expected resulting `GFA_TEST_AUTO_ISSUED_CERTIFICATE_NUMBER`; then run `npm run type-check`, `npm run build`, and `npx playwright test tests/playwright/10-certificate-contract-v2.spec.ts`. The Preview test must prove that a qualifying completion is automatically issued, its private PDF is created, the exact number verifies as active, a duplicate event creates no second certificate or number, and public verification discloses no learner identity or document URL. Do not point these fixtures at production.

The protected administrator Issue route remains only for historical/manual-exception records already in `PENDING_REVIEW`; it is not part of the new qualifying-completion flow. Revoke and supersede remain protected GFA lifecycle operations. BetterDriver’s later presentation implementation must request a fresh scoped GFA handoff whenever the authenticated driver opens My Certificate, so an active certificate remains repeatedly viewable and downloadable while each individual handoff code remains short-lived and one-time.
